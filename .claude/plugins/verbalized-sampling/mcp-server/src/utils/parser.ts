 ...
 */
export function parseVSResponse(response: string): ParseResult {
  try {
    ...
    for (let i = 0; i < parsed.items.length; i++) {
      ...
      items.push({
        ...
        tradeoffs: Array.isArray(raw.tradeoffs)
          ? raw.tradeoffs.filter((t: unknown) => typeof t === 'string')
          : undefined,
      });
    }
 ...
 */
export function validateTauConstraint(items: Item[], tau: number): {
  valid: boolean;
  violations: number[];
} {
 ...
 */
export function applyTauConstraint(items: Item[], tau: number): Item[] {
 ...
 */
export function formatDistribution(items: Item[]): string {
  const lines: string[] = [];
